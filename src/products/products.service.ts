...
export class ProductService {
  ...
  async createProduct(payload: CreateProductInput) {
    try{
    let product= await this.prisma.product.findUnique({where:{name:payload.name}})
    if (product){
      throw new Error(`Product name already exist.`)
    }
    if (!product){
             product = await this.prisma.product.create({ data: { name: payload.name } });
    }
    return product
  }catch(error){
  ...
  async getBatchDetails(payload:GetBatchsearchInput){
try{
    let product=await this.prisma.product.findUnique({where:{name:payload.name}})
